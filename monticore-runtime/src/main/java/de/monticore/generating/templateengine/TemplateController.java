...
public class TemplateController {
  ...
  private final TemplateControllerConfiguration.TemplateLogger log;
  ...
  protected TemplateController(TemplateControllerConfiguration tcConfig, String templatename) {
    ...
    this.log = tcConfig.getTemplateLogger();
  ...
  public void writeArgs(final String templateName, final Path filePath, final ASTNode ast,
    ...
    if (config.isTracing()) {
      content.insert(0, config.getCommentStart() + " generated from model " + config.getModelName()
    ...
    if (filePath.isAbsolute()) {
      ...
      completeFilePath = Paths.get(config.getTargetDir().getAbsolutePath(), filePath.toString());
    }
  ...
  StringBuilder runInEngine(List<Object> passedArguments, Template template, ASTNode ast) {
    ...
    if (template != null) {
      ...
      d.put(TemplateControllerConstants.LOG, log);
