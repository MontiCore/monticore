    ...
    if (node.getTFRule().isPresentTFAssignments()) {
      ...
      for (ASTAssign a : assignments.getAssignList()) {
        ...
        if (!state.getRHSOnlyVars().contains(a.getVariable())) {
          Log.error(String.format("0xF0C06${service.getGeneratedErrorCode(classname)} Schema variable %s not a schema variable.", a.getVariable()), a.get_SourcePositionStart());
        }
      }
      ...
      if(!assignableVars.isEmpty()){
        Log.warn("0xF0C03${service.getGeneratedErrorCode(classname)} Schema variables for names appearing on the right side of a modification should be assigned in the 'Assign' block\"\n"
