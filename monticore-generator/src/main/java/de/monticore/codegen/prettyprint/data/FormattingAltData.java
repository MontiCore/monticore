...
public class FormattingAltData implements Comparable<FormattingAltData> {

  /**
   * Data for inner rule components, in grammar order. Each element is either a
   * FormattingPPComponent (leaf) or a nested FormattingBlockData (optional/star/plus group).
   * No shared interface is used on purpose; consumers must instanceof-check.
   */
  protected final List<FormattingPPGuardComponent> componentList = new ArrayList<>();
  ...
  public FormattingAltData(ASTNode node) {
    this.node = node;
  }

  public List<FormattingPPGuardComponent> getComponentList() {
   ...
   */
  @Override
  public int compareTo(FormattingAltData o) {
