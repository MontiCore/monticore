...
public class ConstantTypeTranslation implements
  ...
  public Link<ASTMCGrammar, ASTCDCompilationUnit> apply(
    ...
    for (Link<ASTConstant, ASTCDAttribute> link : rootLink.getLinks(ASTConstant.class,
      ...
      /* if (!att.isIterated()) { if (attributeType.getEnumValues().size() > 1) {
       * attribDef.setObjectType("int"); } else { attribDef.setObjectType("boolean"); } } else { if
       * (attributeType.getEnumValues().size() > 1) {
       * attribDef.setObjectType("java.util.List<Integer>"); } else {
