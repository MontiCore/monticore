...
public class DeprecatedTranslation implements
  ...
  protected void translateProd(ASTProd prod, ASTCDType cdType,
    ...
    List<ASTGrammarAnnotation> annos = prod.getGrammarAnnotationList().stream().filter(
        ASTGrammarAnnotation::isDeprecated).toList();
    String message;
    if (annos.isEmpty()) {
      ...
      message = annos.getFirst().isPresentMessage() ? astGrammar.getGrammarAnnotation().getMessage() : "";
    }
    if (!message.isEmpty()) {
