...
public class MillDecorator extends AbstractDecorator<ASTCDCompilationUnit, ASTCDClass> {
  ...
  public ASTCDClass decorate(ASTCDCompilationUnit compilationUnit) {
    ...
    ASTType millType = this.getCDTypeFactory().createTypeByDefinition(millClassName);
    ...
    ASTCDConstructor constructor = this.getCDConstructorFactory().createConstructor(PROTECTED, millClassName);

    ASTCDAttribute millAttribute = this.getCDAttributeFactory().createAttribute(PROTECTED_STATIC, millType, MILL_INFIX);

    //add mill attribute for each class
    List<ASTCDAttribute> attributeList = new ArrayList<>();
    for (String attributeName : getAttributeNameList(astcdClassList)) {
      attributeList.add(this.getCDAttributeFactory().createAttribute(PROTECTED_STATIC, millType, MILL_INFIX + attributeName));
    }
  ...
  protected List<ASTCDMethod> addBuilderMethods(List<ASTCDClass> astcdClassList) {
    ...
    for (ASTCDClass astcdClass : astcdClassList) {
      ...
      ASTType builderType = this.getCDTypeFactory().createSimpleReferenceType(astName + BUILDER);

      // add public static Method for Builder
      ASTCDMethod builderMethod = this.getCDMethodFactory().createMethod(PUBLIC_STATIC, builderType, StringTransformations.uncapitalize(astName) + BUILDER);
      ...
      ASTCDMethod protectedMethod = this.getCDMethodFactory().createMethod(PROTECTED, builderType, "_" + StringTransformations.uncapitalize(astName) + BUILDER);
    ...
    for (CDSymbol superSymbol : superSymbolList) {
      ...
      if (astNode.isPresent() && astNode.get() instanceof ASTCDDefinition) {
        ...
        for (ASTCDClass superClass : superDefinition.getCDClassList()) {
          ...
          //add builder method
          ASTCDMethod createDelegateMethod = this.getCDMethodFactory().createMethod(PUBLIC_STATIC, superAstType, StringTransformations.uncapitalize(superClass.getName()) + BUILDER);
