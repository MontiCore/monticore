# Getting Started with MontiCore {#ch:02gettingstarted}

*co-authored by Oliver Kautz*

This chapter describes the technical installation and usage of MontiCore
for language developers. This chapter further inspects a simple example
grammar and the Java classes and other artifacts generated from this
grammar. After installing MontiCore as described in this chapter, it can
be used to develop new modeling languages and generators as described in
subsequent chapters.

MontiCore provides a command line interface (CLI) tool and can easily be used with Gradle. The
Gradle integration enables developers to easily employ MontiCore in
commonly used integrated development environments (IDEs), such as Eclipse and
IntelliJ IDEA. We strongly recommend to work through the section about the CLI
tool first. The CLI section contains information about an example
MontiCore project and the files generated by MontiCore. It also shortly
explains some key features of MontiCore.

A potentially newer explanation can be found on the MontiCore website.
...
Chapter 16. More information about the example `Automata` language are available
in Section 182.1.
...
www.monticore.de/gettingstarted   // Introductory tutorial
```
</div>

## Prerequisites: Installing the Java Development Kit {#sec:preq}
...
- Test whether the setup was successful. Open a command line shell in
    ...
    JDK (e.g., `javac 1.8.0_192`), then the setup was successful.
...
## Install and Use the MontiCore Command Line Interface {#sec:02startcli}
...
### Inspect the Example Grammar {#sec:examinationcli}
...
modeling language (cf. Chapter 4), which is used by developers for modeling
...
followed by the grammar's name (l. 1). In this example, the grammar is
...
table infrastructure. Details can be found in Chapter 4 and Chapter 9. 
For example, the
...
regular expressions. Details can be found in Chapter 4. The right-hand side of
...
described in Chapter 4. However, the example grammar does not use these concepts.
...
syntax classes as detailedly described in Section 5.10.
...
Details about the generation of AST classes can be found in (cf. Chapter 5). For
...
nonterminal (cf. Listing 2.2, l. 3) and the class `ASTAutomatonBuilder` is the
...
contains the attribute <code>symbol</code> (see Chapter 9).</p>
...
Details about the generated parsers and their uses are described in Chapter 6.
...
described in Chapter 9. The symbol table infrastructure is used for resolving
...
![](/docs/img/GenAutomataSymbols.png)
...
<ul>
  <li>adding a <em>symbol rule</em> in the grammar (described in Chapter 9) or</li>
...
explained in Chapter 7 and Chapter 9.
...
![](/docs/img/GenAutomataVisitors.png)
...
Details about the generated visitor infrastructure are described in Chapter 8.
...
more detailed explanation consider reading Chapter 8.
...
-   `hwc/automata/_symboltable` contains handwritten extensions of the
    ...
    handwritten symbol table infrastructure extensions are described in Chapter 9.
...
public void run(String[] args) {
  ...
  ASTAutomaton ast = parse(model); (*@ \label{AT:1} @*)
  Log.info(model + " parsed successfully!", "AutomataTool");

  // setup the symbol table
  IAutomataArtifactScope modelTopScope =  (*@ \label{AT:2} @*)
    createSymbolTable(ast);
  
  // can be used for resolving names in the model
  Optional<StateSymbol> aSymbol = (*@ \label{AT:3} @*)
  ...
  // setup context condition infrastructure (*@ \label{AT:4} @*)
  ...
  // check the CoCos
  checker.checkAll(ast); (*@ \label{AT:5} @*)

  // Now we know the model is well-formed and start backend

  // store artifact scope and its symbols (*@ \label{AT:6} @*)
  AutomataSymbols2Json deser = new AutomataSymbols2Json();
  deser.store(modelTopScope, args[1]); (*@ \label{AT:7} @*)

  // analyze the model with a visitor
  CountStates cs = new CountStates(); (*@ \label{AT:8} @*)
  AutomataTraverser traverser = AutomataMill.traverser();
  traverser.add4Automata(cs);
  ast.accept(traverser);
  Log.info("Automaton has " + cs.getCount() + " states.", 
    "AutomataTool"); (*@ \label{AT:9} @*)

  // execute a pretty printer (*@ \label{AT:10} @*)
  ...
  Log.info("Pretty printing automaton into console:", 
           "AutomataTool");
  // print the result
  Log.println(pp.getResult()); (*@ \label{AT:11} @*)
}
```
...
-   executes context conditions (ll. 40-49),
-   stores the symbol table by using the serialization (ll. 53-55),
-   executes the visitor for counting the states (ll. 58-63), and
-   pretty prints the model to the standard output (ll. 65-72).
...
## Using MontiCore via Gradle From the Command Line {#sec:MCGradle}

It is possible to execute MontiCore via the MontiCore plugin. A detailed
description about using the MontiCore Gradle plugin is given in Chapter 16. This
section describes the execution of MontiCore via a Gradle plugin from
the command line shell by example.
...
## Using MontiCore in Eclipse {#sec:MCEclipse}
...
### Setting up Eclipse {#sec:SettingUpEclipse}
...
### Importing the Example {#sec:ImportingTheExample}
...
### Running MontiCore {#sec:runmvneclipse}
...
### Setting up IntelliJ IDEA {#sec:SettingUpIntelliJ}
...
### Importing the Example {#sec:ImportingTheExampleIntelliJ}
...
### Running MontiCore {#sec:runmvnintellij}
