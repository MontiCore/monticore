...
- Install a JDK with version 11 provided by Oracle or OpenJDK.
...
- *(Optional)* Install [Gradle](https://gradle.org/install/) version 7.6.4. 
...
=== "Gradle"
    * Download the example `Automata` MontiCore project for Gradle:
    ...
    ```
    * Unzip the archive. The unzipped files include a directory called
    `automaton-master` containing the Gradle build script 
    along with the directories
    `src/main/grammars` containing the `Automata.mc4` grammar,
    `src/main/java` containing both handwritten `Automata` DSL infrastructure 
    and handwritten code that is incorporated into the generated code,
    and `src/test/resources` containing an example model of the `Automata` DSL.
    ```
    ...
    *Note:* When mentioned, this document will specify file locations based
    on the CLI project setup.
    The CLI directories `src` and `hwc` are both represented by the same 
    `src/main/java` directory using Gradle.


The CLI and Gradle setups both work on the same project,
 albeit with a different directory structure.
...
Listing 2.5 depicts an example model conforming to the `Automata` grammar in its
concrete syntax. 
...
=== "Gradle"
    ...
    The [MontiCore standard library](https://git.rwth-aachen.de/monticore/monticore/-/tree/dev/monticore-grammar/src/main),
    containing grammars and Java classes, is imported via the `de.monticore:monticore-grammar` dependency.
    More information about the standard library can be found in
    [Chapters 17- 20 of the handbook](https://www.monticore.de/handbook.pdf).
...
2.  Java source files for the corresponding DSL infrastructure are
    generated into the default output directory `out`.  This infrastructure
...
nonterminal (cf. Listing 2.3) and the class `ASTAutomatonBuilder` is the
...
For instance, Figure 2.7 depicts parts of the generated `AST` infrastructure for
...
interfaces and classes are depicted in Figure 2.17.
...
`CountStates.java`. The class is depicted in Listing 2.23. It implements a simple
...
Listing 2.24 depicts the class `AtLeastOneInitialAndFinalState`. The class implements
...
Listing 2.25 presents the implementation of the class `TransitionSourceExists`. The
...
Listing 2.26 presents the implementation of the method `parse` of the `AutomataTool` 
...
=== "Gradle"
    ...
    **With cmd on Windows**
    ...
    Executing the command runs the `Automata` DSL tool.
