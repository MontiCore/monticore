...
public class BasicFloatLiteralRange implements MCCommonLiteralsASTBasicFloatLiteralCoCo {
  ...
  public BasicFloatLiteralRange(){
    this.min = BigDecimal.valueOf(-Float.MAX_VALUE);
    this.max = BigDecimal.valueOf(Float.MAX_VALUE);
  }

  public BasicFloatLiteralRange(BigDecimal min, BigDecimal max){
    this.min = min;
    this.max = max;
  }
