...
public class SignedBasicLongLiteralRange implements MCCommonLiteralsASTSignedBasicLongLiteralCoCo {
  ...
  public SignedBasicLongLiteralRange(){
    this.min = BigInteger.valueOf(Long.MIN_VALUE);
    this.max = BigInteger.valueOf(Long.MAX_VALUE);
  }

  public SignedBasicLongLiteralRange(BigInteger min, BigInteger max){
    this.min = min;
    this.max = max;
  }
