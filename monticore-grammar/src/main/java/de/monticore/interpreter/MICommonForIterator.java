...
public class MICommonForIterator implements MIForIterator {
  ...
  Optional<ASTForInit> initNode;
  Optional<ASTExpression> condition;
  ...
  public MIValue checkCondition(ModelInterpreter interpreter) {
  ...
  public MIValue increment(ModelInterpreter interpreter) {
  ...
  public MIValue execute(ModelInterpreter interpreter, ASTMCStatement body) {
    ...
    while (conditionResult.isBoolean() && conditionResult.asBoolean()) {
      ...
      if (statementResult.isFlowControlSignal() && !statementResult.isContinue()) return statementResult;
