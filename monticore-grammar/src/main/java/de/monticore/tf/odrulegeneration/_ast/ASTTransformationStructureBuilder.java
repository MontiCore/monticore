...
public class ASTTransformationStructureBuilder extends ASTTransformationStructureBuilderTOP {
  ...
  public ASTTransformationStructureBuilder setFoldingHash(
      HashMap<String, List<String>> foldingHash) {
  ...
  public ASTTransformationStructure build() {
    ...
    if (!isValid()) {
      ...
      if (pattern == null) {
        Log.error(
            "0xA7232 pattern of type de.monticore.tf.odrulegeneration._ast.ASTPattern must not be null");
      }
      if (replacement == null) {
        Log.error(
            "0xA7233 replacement of type de.monticore.tf.odrulegeneration._ast.ASTReplacement must not be null");
      }
