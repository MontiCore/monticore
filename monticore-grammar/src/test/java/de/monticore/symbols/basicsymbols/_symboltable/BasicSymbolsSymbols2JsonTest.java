...
public class BasicSymbolsSymbols2JsonTest {
  ...
  public void performRoundTripSerialization(IBasicSymbolsArtifactScope scope){
    ...
    assertSame(type.get().getSpannedScope(), variable2.get().getType().asTypeVariable().getTypeVarSymbol().getEnclosingScope());
    assertSame(deserializedType.get().getSpannedScope(), deserializedVariable2.get().getType().asTypeVariable().getTypeVarSymbol().getEnclosingScope());
