...
public class FilterStream<T> extends UntimedStream<T> {
  protected final UntimedStream<T> stream;
  protected final Function1<Boolean, T> predicate;

  public FilterStream(UntimedStream<T> stream, Function1<Boolean, T> predicate) {
    this.stream = stream;
  ...
  public Tuple2<Optional<T>, UntimedStream<T>> _internal_next() {
    ...
    tail = new FilterStream<>(tail, predicate);

    if (predicate.apply(optHead.get())) {
